...
missed. The follow-up checker fix is merged into Forge `main` as error rule
`OF305` and will ship in the next release; the installed `v0.3.0` checker does
not contain that rule. This evidence verifies the
