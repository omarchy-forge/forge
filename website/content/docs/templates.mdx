...
Agent-ready scaffolding is implemented on Forge `main` for the next release; it
is not included in the public `v0.2.0` binary yet. When using a current source
build, create the same reviewed template for Omarchy's configured coding agent:
...
The opt-in flag adds `FORGE_SPEC.md` and `AGENTS.md`. Complete every `TODO` in
the specification before involving an agent. The generated guardrails restrict
the agent to the project, preserve the manifest and runtime contracts, allow
static validation, and reserve QML execution and installation for the user.

Forge does not choose a model, contact an AI service, collect a prompt, or
launch an agent. After reviewing the two files and committing the clean
scaffold, an Omarchy user may explicitly hand it to their configured agent:
