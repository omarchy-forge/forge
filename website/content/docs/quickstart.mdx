...
uses an AMD/Intel or ARM processor, downloads only the matching Forge `v0.3.0`
...
directory, and returns the terminal to your home directory. If a new terminal
says that `omaforge` is not found, run it as `$HOME/.local/bin/omaforge` or add
